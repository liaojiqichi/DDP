...
class LLMEngine:
 ...
 #       with open('/DDP/data.txt','r') as f:

  #         last_line = f.readlines()[-1]
   
   #     if  last_line == "Receive\n":
     #        self.record1()
...
#        with open('/DDP/data.txt','r') as f:
 #           last_line = f.readlines()[-1]
  #      if  last_line == "Receive\n":
   #         self.record2()
    #        self.state_logger.log(self._get_stats(scheduler_outputs))
    ...
    def record1(self) ->None:
        now_time = datetime.now().time()
        now = time.monotonic()
        should_record = now - self.last_recording_time >= 2
        ...
        cache_block_size = CacheEngine.get_cache_block_size(
                block_size, "auto", self.model_config, self.parallel_config)
        num_free_gpu_block = (self.scheduler.block_manager.get_num_free_gpu_blocks())
        used_gpu = total_num_gpu_blocks - num_free_gpu_block
        GPU_used_memory = used_gpu*cache_block_size
        CPU_used_memory = num_used_cpu_blocks*cache_block_size
        #self.x.append(used_memory)
        with open(r'/DDP/data.txt', 'w+') as fp:
            fp.write("GPU_Used:%d  " %used_gpu)
            fp.write("CPU_Used_Memory:%d  " %num_used_cpu_blocks)
            fp.write("Block_Size:%d  " %cache_block_size)
            fp.write("Time is:%s  \n" %now_time)
        fp.close()
        logger.info("used msmory is %d" %GPU_used_memory)
            #logger.info("the time is %s", now)
        self.last_recording_time = now
    
    def record2(self) ->None:
        now_time = datetime.now().time()
        now = time.monotonic()
        total_num_cpu_blocks = self.cache_config.num_cpu_blocks
        if total_num_cpu_blocks > 0:
        ...
        GPU_used_memory = used_gpu*cache_block_size
        CPU_used_memory = num_used_cpu_blocks*cache_block_size
        with open(r'/DDP/data.txt', 'a+') as fp:
            fp.write("GPU_Used:%d  " %used_gpu)
            fp.write("CPU_Used_Memory:%d  " %num_used_cpu_blocks)
            fp.write("Block_Size:%d  " %cache_block_size)
            fp.write("Time is:%s  \n" %now_time)
