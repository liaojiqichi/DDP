...
class LLMEngine:
    ...
    def __init__(
        ...
        self.last_recording_time = datetime.now().time()
    ...
    def record1(self) ->None:
        ...
        # now = time.monotonic()
        should_record = now.second - self.last_recording_time.second >= 0.1
        ...
        with open(r'/DDP/data.txt', 'a+') as fp:
            fp.write("GPU_Used_Memory:%d  " %GPU_used_memory)
            fp.write("CPU_Used_Memory:%d  " %CPU_used_memory)
            fp.write("Time is:%s\n" %now)
        fp.close()
        #logger.info("used msmory is %s", used_memory)
